...
class AuteurRepository extends ServiceEntityRepository
{
    ...
    public function search(string $searchValue): array {
        return $this->createQueryBuilder('a') //SELECT * FROM auteur AS a
            ->where('a.nom LIKE :searchValue') // WHERE a.nom LIKE '%searchValue%'
            ->orWhere('a.prenom LIKE :searchValue') // OR a.prenom LIKE '%searchValue%'
            ->orWhere('a.pseudo LIKE :searchValue')
            ->setParameter('searchValue', '%'.$searchValue.'%') // SET searchValue = '%searchValue%'
            ->orderBy('a.nom', 'ASC')
            ->getQuery() // Executer
            ->getResult() // Retourner sous forme de tableau
        ;
    }
